...
doog         ~user\util\general\doog.ijs
)

NB. require'yaml'
require 'loadp'  NB.  Loads a script and prints the names it defined.
...
load 'OS\commandLineParser.ijs' ,~ 0 {:: utilspath

NB.  Require this so that we can save/load workspaces
load 'environment\jworkspace.ijs' ,~ 0 {:: utilspath

NB.  Require this so that we use formatTimeSpan
load 'time\formattimespan.ijs' ,~ 0 {:: utilspath

NB.  Require this so that we can shorten the path in "recent files"
load 'environment\reduce_prefix.ijs' ,~ 0 {:: utilspath

NB.  Require this so that we can shorten the path in "recent files"
load 'environment\nameScope.ijs' ,~ 0 {:: utilspath

NB.  Load all of Oleg's useful utils.
load (#~ endsWith&'x') listFiles 'contrib\oleg\*.ijs' ,~ 0 {:: utilspath
