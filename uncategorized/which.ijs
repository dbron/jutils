	...
	if. 0 -: > {. 'ok handle' =. regcreatekey HKEY_LOCAL_MACHINE;'SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths';1 do.
		...
		if. -. 0 -: nc {. ;: 'MAX_PATH_which_' do.
			...
			if. * # d =. winconst 'MAX_PATH'[ require'winapi' do.
				MAX_PATH_which_ =:  > {: , d
			elseif. -. _1 -: data =. fread 'C:\Program Files\Microsoft Visual Studio\VC98\CRT\SRC\STDLIB.H' do.	
...
normalizeSpaces		=.    (' '&collapseMultiples    trim    (TAB,LF) msr '  ' ) ats
trimmy		        =.    + 1 : ( ('(9 32{a.)';parenclose ntt '';');') replace trim vtt2 )
...
PORKENDOOFY=: + 1 : (parenclose cTojDef	vtt2)

callC				=.  2 : ('(m. (, '' ''&,@:' , (parenclose cTojDef vtt2) , ' ) n.)&cd')
...
REG_QUERY_INFO_KEY=: 'RegQueryInfoKeyA' win32api NB. 'Advapi32' callC (noun define)
...
REG_ENUM_VALUE =: 'RegEnumValueA' win32api NB. 'Advapi32' callC (noun define)
